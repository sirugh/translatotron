...
// TODO: come back and use csv-parse
// const newTranslations = parse(fs.readFileSync(legacyFilePath, "utf8"), {
// });

fs.createReadStream(legacyFilePath)
  .pipe(csv(["key", "translation"]))
  .on("data", ({ key, translation }) => {
    // Spread each row into results. May overwrite if duplicates.
    legacyTranslations[key] = translation;
  })
  .on("end", () => {
    console.log("\n---- LEGACY TRANSLATIONS ----");
      ...
      files.forEach((file) => {
        ...
        Object.keys(newTranslations).forEach((key) => {
          if (!generatedKeys.includes(key))
            generated[key] = newTranslations[key];
        });
